...
{
	unsigned len = min(t->len - tspi->cur_pos, BB_LEN * tspi->cur_bpw);
	...
	val &= ~WORD_SIZE(~0);
	val |= WORD_SIZE(len / tspi->cur_bpw - 1);
	spi_writel(tspi, val, SLINK_COMMAND);

	for (i = 0; i < len; i += tspi->cur_bpw) {
		val = 0;
		for (j = 0; j < tspi->cur_bpw; j++ )
			val |= tx_buf[i + j] << j * 8;

		spi_writel(tspi, val, SLINK_TX_FIFO);
	}

	tspi->rx_dma_req.size = (len / tspi->cur_bpw ) * 4;
				    ...
				    struct spi_transfer *t)
{
	...
	tspi->cur_bpw = bytes_per_word(bits_per_word);

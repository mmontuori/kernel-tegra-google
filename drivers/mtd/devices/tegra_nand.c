 ...
 *
 * NV Tegra APxx on-chip NAND flash controller driver
 *
...
find_nand_flash_device(int dev_id)
{
	...
	for (; dev->name && dev->id != dev_id; ++dev);
...
find_nand_flash_vendor(int vendor_id)
{
	...
	for (; vendor->id && vendor->id != vendor_id; ++vendor);
...
tegra_nand_scan(struct mtd_info *mtd, int maxchips)
{
	...
	select_chip(info, 0);
	if ((err = tegra_nand_cmd_readid(info, &tmp)) != 0)
		goto out_error;
	...
	if ((dev_info = find_nand_flash_device(dev_id)) == NULL) {
	...
	if ((vendor_info = find_nand_flash_vendor(vendor_id)) == NULL) {

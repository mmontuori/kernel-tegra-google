...
struct avp_info {
	...
	u32				msg;
	void __iomem			*msg_to_avp;
	void __iomem			*msg_from_avp;
				      ...
				      struct msg_data *raw_msg)
{
	...
	if (avp_debug_mask & AVP_DBG_TRACE_XPC_CONN) {
		pr_info("%s: got disconnect (%x)\n", __func__,
			disconn_msg->port_id);
				   ...
				   struct msg_data *raw_msg)
{
	...
	if (avp_debug_mask & AVP_DBG_TRACE_XPC_CONN) {
		pr_info("%s: got connect (%x)\n", __func__, conn_msg->port_id);
...
static int tegra_avp_open(struct inode *inode, struct file *file)
{
	...
	if (cmpxchg(&avp->opened, false, true) == true)
		return -EBUSY;
...
{
	u32 *avp_suspend_done = avp->iram_backup_data + TEGRA_IRAM_SIZE;
...
err:
	 ...
	 *
	 * for now, just reenable the irq to keep it balanced */
	enable_irq(avp->mbox_from_avp_pend_irq);
	return ret;
}
...
static int tegra_avp_probe(struct platform_device *pdev)
{
	...
	avp->nvmap_drv = nvmap_create_client(nvmap_dev);
	 ...
	 * us that it is done suspending is the resume address */

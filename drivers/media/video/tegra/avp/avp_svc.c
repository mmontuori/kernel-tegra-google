...
static const char *avp_clk_names[] = {
	[CLK_REQUEST_VDE]	= "vde",
	[CLK_REQUEST_VCP]	= "vcp",
	[CLK_REQUEST_BSEA]	= "bsea",
};
#define NUM_AVP_CLOCKS		ARRAY_SIZE(avp_clk_names)
...
struct avp_svc_info {
	struct avp_clk			clks[NUM_AVP_CLOCKS];
	/* used for dvfs */
	struct clk			*sclk;
			       ...
			       size_t len)
{
	...
	if (IS_ERR(handle)) {
		pr_err("avp_svc: unknown remote handle %x\n", msg->handle_id);
		err = AVP_ERR_EACCES;
		goto out;
	}
	...
	if (err) {
		pr_err("avp_svc: can't allocate for handle %x (%d)\n",
		       msg->handle_id, err);
		err = AVP_ERR_ENOMEM;
	}
				 ...
				 size_t len)
{
	...
	handle = nvmap_duplicate_handle_id(avp_svc->nvmap_remote,
					   msg->handle_id);
	if (IS_ERR(handle)) {
		pr_err("avp_svc: can't duplicate handle for id %x (%d)\n",
...
static struct avp_clk *module_to_clk(struct avp_svc_info *avp_svc, u32 id)
{
	int idx;

	/* XXX: could probably do it with a map */
	switch (id) {
	case AVP_MODULE_ID_VCP:
		idx = CLK_REQUEST_VCP;
		break;
	case AVP_MODULE_ID_BSEA:
		idx = CLK_REQUEST_BSEA;
		break;
	case AVP_MODULE_ID_VDE:
		idx = CLK_REQUEST_VDE;
		break;
	default:
		return NULL;
				...
				size_t len)
{
	...
	pr_info("avp_svc: module reset: %x\n", msg->module_id);

	aclk = module_to_clk(avp_svc, msg->module_id);
	if (!aclk) {
				...
				size_t len)
{
	...
	pr_info("avp_svc: module clock: %x %s\n", msg->module_id,
		msg->enable ? "on" : "off");
	aclk = module_to_clk(avp_svc, msg->module_id);
	if (!aclk) {
		pr_err("avp_svc: unknown module clock requested: %x\n",
		       msg->module_id);
	...
	if (msg->enable) {
		...
		BUG_ON(avp_svc->clk_reqs & aclk->bit);

		clk_enable(aclk->clk);
		avp_svc->clk_reqs |= aclk->bit;
	} else {
		BUG_ON(!(avp_svc->clk_reqs & aclk->bit));
...
int avp_svc_start(struct avp_svc_info *avp_svc)
{
	...
	avp_svc->nvmap_remote = nvmap_create_client(nvmap_dev);
...
void avp_svc_stop(struct avp_svc_info *avp_svc)
{
	...
	mutex_lock(&avp_svc->clk_lock);
	for (i = 0; i < NUM_AVP_CLOCKS; i++)
		if (avp_svc->clk_reqs & avp_svc->clks[i].bit) {
			pr_info("%s: remote left '%s' clock on\n", __func__,
				avp_clk_names[i]);
			clk_disable(avp_svc->clks[i].clk);
		}
	avp_svc->clk_reqs = 0;
	mutex_unlock(&avp_svc->clk_lock);
}
				  ...
				  struct trpc_node *rpc_node)
{
	...
	BUILD_BUG_ON(NUM_AVP_CLOCKS > BITS_PER_LONG);

	avp_svc->sclk = clk_get(&pdev->dev, "sclk");
	for (i = 0; i < NUM_AVP_CLOCKS; i++) {
		avp_svc->clks[i].bit = 1 << i;
		avp_svc->clks[i].clk = clk_get(&pdev->dev, avp_clk_names[i]);
		if (IS_ERR(avp_svc->clks[i].clk)) {
			ret = PTR_ERR(avp_svc->clks[i].clk);

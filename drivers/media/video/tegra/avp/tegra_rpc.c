...
static struct trpc_port *rpc_port_alloc(const char *name, gfp_t gfp_mask)
{
	struct trpc_port *port;
	int i;

	port = kzalloc(sizeof(struct trpc_port), gfp_mask);
	if (!port) {
				  ...
				  struct trpc_ep_ops *ops, void *priv)
{
	...
	new_port = rpc_port_alloc(name, GFP_KERNEL);
	if (!new_port) {
...
/* Exported code. This is out enterface to the outside world */

...
#define kmalloc_stack(num, buff)					\
	({								\
		typeof(buff[0]) * bptr = buff;				\
		if (num > ARRAY_SIZE(buff))				\
			bptr = kmalloc(sizeof(*bptr)*num, GFP_KERNEL);	\
		bptr;							\
	})
...
int nvmap_ioctl_pinop(struct file *filp, bool is_pin, void __user *arg)
{
	...
	if (op.count > 1) {
		...
		refs = kmalloc_stack(op.count, on_stack);
...
int nvmap_ioctl_create(struct file *filp, unsigned int cmd, void __user *arg)
{
	...
	if (cmd == NVMAP_IOC_CREATE) {
		r = nvmap_create_handle(client, ALIGN(op.size, PAGE_SIZE));
		if (!IS_ERR(r))
			r->h->orig_size = op.size;
	} else if (cmd == NVMAP_IOC_FROM_ID)
		r = nvmap_duplicate_handle_id(client, op.id);
	else
		return -EINVAL;
...
int nvmap_ioctl_cache_maint(struct file *filp, void __user *arg)
{
	...
	err = cache_maint(client, vpriv->h, start, end, op.op);
out:
	up_read(&current->mm->mmap_sem);
	return err;
}

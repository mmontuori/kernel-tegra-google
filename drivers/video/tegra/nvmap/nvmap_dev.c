...
struct nvmap_handle_ref *_nvmap_validate_ref_locked(struct nvmap_client *c,
						    unsigned long ref)
{
	...
	while (n) {
		...
		if ((unsigned long)r->h == ref) {
			if (WARN_ON(r->h->poison != NVMAP_POISON))
				return NULL;
			return r;
		} else if (ref > (unsigned long)r->h)
			n = n->rb_right;
		else
			n = n->rb_left;
	}
				  ...
				  struct nvmap_heap_block *b, size_t len)
{
	...
	void *addr;
	unsigned long kaddr;
	unsigned long phys = b->base;
	unsigned long end = b->base + len;
	...
	outer_flush_range(b->base, b->base + len);
					      ...
					      unsigned int prot)
{
	...
	list_for_each_entry(n, &dev->heaps, heap_list) {
		struct nvmap_heap_block *b;

		if (!(n->heap_bit & usage))
			continue;

		b = nvmap_heap_alloc(n->carveout, len, align, prot);
		if (b) {
			/* flush any stale data that may be left in the
			 * cache at the block's address, since the new
			 * block may be mapped uncached */
			if (nvmap_flush_heap_block(client, b, len)) {
				nvmap_heap_free(b);
				return NULL;
			} else
				return b;
...
struct nvmap_client *nvmap_client_get(struct nvmap_client *client)
{
	...
	if (WARN_ON(!atomic_add_unless(&client->ref, 1, 0)))
		return NULL;
...
{
	struct nvmap_client *c = ERR_PTR(-EFAULT);

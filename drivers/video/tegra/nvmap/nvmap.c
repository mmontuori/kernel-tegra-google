 ...
 * this function. */
static int pin_locked(struct nvmap_client *c, struct nvmap_handle *h)
{
...
static int wait_pin_locked(struct nvmap_client *c, struct nvmap_handle *h)
{
	int ret = 0;
	...
	if (ret) {
		ret = wait_event_interruptible(c->share->pin_wait,
					       !pin_locked(c, h));
	}
...
static int handle_unpin(struct nvmap_client *c, struct nvmap_handle *h)
{
	int ret = 0;
	...
	if (atomic_read(&h->pin) == 0) {
		nvmap_err(c, "%s unpinping unpinned handle %p\n",
	...
	if (!atomic_dec_return(&h->pin)) {
		if (h->heap_pgalloc && h->pgalloc.area) {
			...
			nvmap_mru_insert_locked(c->share, h);
			ret = 1;
		}
	}
		     ...
		     unsigned int nr, const unsigned long *ids)
{
	...
	for (i = 0; i < nr; i++) {
		...
		r = _nvmap_validate_ref_locked(client, ids[i]);
		if (r) {
			struct nvmap_handle *h = r->h;
			int e = atomic_add_unless(&r->pin, -1, 0);
		  ...
		  unsigned int nr, const unsigned long *ids)
{
	...
	for (i = 0; i < nr && !ret; i++) {
		...
		} else if (h[i]->poison != NVMAP_POISON)
			ret = -EINVAL;
				 ...
				 int nr, struct nvmap_handle *gather)
{
	...
	for (i = 0; i < nr; i++) {
		...
		BUG_ON(patch->poison != NVMAP_POISON);
					...
					int nr, struct nvmap_handle **h)
{
	...
	for (i = 0; i < nr; i++) {
		...
		r = _nvmap_validate_ref_locked(client, arr[i].pin_mem);
		...
		if (r->h->flags & NVMAP_HANDLE_VISITED)
			continue;

		r->h->flags |= NVMAP_HANDLE_VISITED;

		h[count] = nvmap_handle_get(r->h);
		BUG_ON(!h[count]);
		count++;
	}
...
unsigned long nvmap_pin(struct nvmap_client *client, struct nvmap_handle_ref *r)
{
	struct nvmap_handle *h;
	unsigned long phys;
	int ret = 0;

	h = nvmap_handle_get(r->h);
	if (WARN_ON(!h))
		return -EINVAL;
			 ...
			 struct nvmap_handle **h, int nr)
{
	...
	for (i = 0; i < nr; i++) {
		if (WARN_ON(!h[i]))
			continue;
		BUG_ON(h[i]->poison != NVMAP_POISON);
		do_wake |= handle_unpin(client, h[i]);
	}
...
void *nvmap_mmap(struct nvmap_handle_ref *r)
{
	...
	adj_size += h->size;
	adj_size = ALIGN(adj_size, PAGE_SIZE);
...
void nvmap_munmap(struct nvmap_handle_ref *r, void *addr)
{
	...
	if (h->heap_pgalloc && (!h->pgalloc.contig ||
				PageHighMem(h->pgalloc.pages[0])))
		vm_unmap_ram(addr, r->h->size >> PAGE_SHIFT);
	else if (!h->heap_pgalloc) {
		struct vm_struct *vm;
		addr -= (r->h->carveout->base & ~PAGE_MASK);

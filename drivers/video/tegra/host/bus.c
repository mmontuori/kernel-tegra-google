...
{
	pr_info("host driver register %s\n", drv->driver.name);
...
int nvhost_device_register(struct nvhost_device *dev)
{
	...
	pr_info("Registering nvhost device '%s'. Parent at %s\n",
...
int nvhost_bus_register(struct nvhost_dev *host)
{
	pr_info("host1x bus register %p\n", host);

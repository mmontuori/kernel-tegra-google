...
static void tegrafb_writel(struct tegrafb_info *tegrafb, u32 val, unsigned long reg)
{
	writel(val, tegrafb->reg_base + reg*sizeof(u32));
}

static u32 tegrafb_readl(struct tegrafb_info *tegrafb, unsigned long reg)
{
...
static int tegra_plat_probe( struct platform_device *pdev )
{
	...
	strlcpy(fb->fix.id, "tegrafb", sizeof(fb->fix.id));

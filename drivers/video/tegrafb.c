...
static inline void tegrafb_writel(struct tegrafb_info *tegrafb, u32 val, unsigned long reg)
{
	writel(val, tegrafb->reg_base + reg*sizeof(u32));
}

static inline u32 tegrafb_readl(struct tegrafb_info *tegrafb, unsigned long reg)
{
	return readl(tegrafb->reg_base + reg*sizeof(u32));
}
...
irqreturn_t tegrafb_irq(int irq, void *ptr)
{
	...
	tegrafb_writel(tegrafb, reg, DC_CMD_INT_STATUS);
...
static int tegra_fb_wait_for_vsync(struct fb_info *fb, unsigned long timeout)
{
	struct tegrafb_info *tegrafb = fb->par;

	tegrafb->wait_condition = 0;

	tegrafb_writel(tegrafb, tegrafb->irq_mask, DC_CMD_INT_STATUS);
	tegrafb_writel(tegrafb, tegrafb->irq_mask, DC_CMD_INT_ENABLE);

	wait_event_interruptible_timeout(tegrafb->frame_wq, tegrafb->wait_condition, timeout);

	tegrafb_writel(tegrafb, 0, DC_CMD_INT_ENABLE);
...
int tegra_fb_pan_display(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	dmac_clean_range(flush_start, flush_start);
	...
	tegrafb_writel(tegrafb, 1<<4, DC_CMD_DISPLAY_WINDOW_HEADER);
	tegrafb_writel(tegrafb, addr, DC_WINBUF_A_START_ADDR);
	tegrafb_writel(tegrafb, (1 << 8) | (1 << 9), DC_CMD_STATE_CONTROL);
	tegrafb_writel(tegrafb, (1 << 0) | (1 << 1), DC_CMD_STATE_CONTROL);
	while (tegrafb_readl(tegrafb, DC_CMD_STATE_CONTROL) & 3) {
		tegra_fb_wait_for_vsync(info, HZ/10);
		count++;
	}
	if (unlikely(count > 1))
...
#ifdef DEBUG
#define DUMP_REG(a) pr_debug("%-32s\t%03x\t%08x\n", #a, a, tegrafb_readl(tegrafb, a));
...
static int tegra_plat_probe( struct platform_device *pdev )
{
	...
	strncpy(fb->fix.id, "tegrafb", sizeof(fb->fix.id));
	...
	reg = tegrafb_readl(tegrafb, DC_CMD_INT_MASK);
	reg |= tegrafb->irq_mask;
	tegrafb_writel(tegrafb, reg, DC_CMD_INT_MASK);
...
static int __init tegra_fb_init( void )
{
	...
	if( e ) {
		pr_info( "nvtegrafb: platform_driver_register failed\n" );
		return e;
	}
	return e;
}

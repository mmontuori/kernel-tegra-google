...
static inline u32 fuse_readl(unsigned long offset)
{
	...
	if (wait_event_timeout(waitq, rd_lock, msecs_to_jiffies(50)) == 0) {
		WARN_ON(1);
		return 0;
	}
	rd_lock = 0;
...
static inline void fuse_writel(u32 value, unsigned long offset)
{
	...
	*((u32 *)fuse_bb) = value;
	...
	tegra_dma_enqueue_req(dma, &req);
	if (wait_event_timeout(waitq, wr_lock, msecs_to_jiffies(50)) == 0)
		WARN_ON(1);
	wr_lock = 0;
#else
...
static int fuse_write(u32 io_param_type, void *data, int size)
{
	u32 reg;
	int ret;
...
int tegra_fuse_io(u32 io_param_type, void *data, int size, int direction)
{
	...
	if (direction) {
		return fuse_write(io_param_type, data, size);
	}
	else {
		mutex_lock(&fuse_lock);
		fuse_reg_unhide();

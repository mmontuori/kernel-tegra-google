...
int tegra_arb_mutex_lock_timeout(enum tegra_arb_module lock, u16 timeout)
{
	...
	wait_event_timeout(arb->arb_gnt_q, arb->arb_gnt_wake[lock],
		jiffies);
	if (arb->arb_gnt_wake[lock] == 0) {
		pr_err("timed out.\n");
		cancel_arb_sem(lock);
...
int tegra_arb_mutex_unlock(enum tegra_arb_module lock)
{
	...
	arb_sema_write(1 << lock, ARB_GRANT_RELEASE);
	ier = arb_gnt_read(ARB_CPU_INT_EN);
	arb_gnt_write(ier & ~(1 << lock), ARB_CPU_INT_EN);

...
{
	int i, j, start_bit = fuse_info_tbl[io_param].start_bit;
	...
	do {
		...
		for (i = 0, j = cur_bit; i < loops; i++, j++) {
			if (val & (BIT(i) << start_bit))
				*dst |= BIT(j);

			nbits--;
			cur_bit++;
			if ((i + start_bit == 31) || (!nbits))
				break;
		}
		if (i < loops) {
			offset += 2;
			...
			dst++;
			start_bit = fuse_info_tbl[io_param].start_bit;
			cur_bit = 0;
			loops = min(nbits, 32);
		}
	} while (nbits > 0);
...
static void set_fuse(enum fuse_io_param io_param, u32 *data)
{
	...
	do {
		val = *data;
		for (i = start, j = start_bit; i < loops; i++, j++) {
			fuse_pgm_mask[offset] |= BIT(j);
			if (val & BIT(i))
				fuse_pgm_data[offset] |= BIT(j);
			nbits--;
			if ((j == 31) || (nbits == 0)) {
				start = ++i;
				start_bit = 0;

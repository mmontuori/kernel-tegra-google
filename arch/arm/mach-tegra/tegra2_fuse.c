...
static void get_fuse(enum fuse_io_param io_param, u32 *out)
{
	...
	u32 *dst = (u32 *)fuse_info_tbl[io_param].addr, val;
	...
	do {
		...
		loops = min(nbits, 32);
		nbits -= loops;
		*dst &= ~((u32)(0xFFFFFFFF >> loops));

		i = start_bit;
		do {
			*dst |= ((val & BIT(i)) >> start_bit);
			i++;
		} while (--loops && (i < 32));
		if (loops > 0) {
			offset += 2;
...
int tegra_fuse_read(enum fuse_io_param io_param, void *data, int size)
{
	...
	if (io_param == SBK_DEVKEY_STATUS) {
		*(u8 *)data = 0;
		...
		get_fuse(SBK, (u32 *)sbk);
		get_fuse(DEVKEY, (u32 *)devkey);
...
static bool fuse_odm_prod_mode(void)
{
	...
	get_fuse(ODM_PROD_MODE, (u32 *)&odm_prod_mode);
	return (odm_prod_mode ? true : false);
}

static void set_fuse(enum fuse_io_param io_param, u32 *data)
{
	int i = 0, start_bit = fuse_info_tbl[io_param].start_bit, ctr;
	int nbits = fuse_info_tbl[io_param].nbits, loops;
	...
	do {
		...
		ctr = start_bit;
		do {
			fuse_pgm_mask[offset] |= (BIT(i) << start_bit);
			fuse_pgm_data[offset] |= ((val & BIT(i)) << start_bit);
			i++;
			ctr++;
		} while (--loops && (ctr < 32));
		if (loops) {
			offset++;
			do {
				fuse_pgm_mask[offset] |= (BIT(i) >>
					(32 - start_bit));
				fuse_pgm_data[offset] |= ((val & BIT(i)) >>
					(32 - start_bit));
				i++;
			} while (--loops);
		}

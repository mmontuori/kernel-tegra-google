...
static inline int ROUND_DOWN(int v, int m)
{
	v /= m;
	v *= m;
	return v;
		...
		bool out_stereo)
{
	...
	*consumed = i;
	...
	D("%s: in_len %d out_len %d consumed %d generated %d\n", __func__,
				...
				int *num_consumed)
{
	...
	*num_consumed *= sizeof(s16);
	...
	D("%s: generated %d, skipped %d, original in fifo %d\n",
			...
			size_t size) /* bytes to write to user buffer */
{
	...
	D("%s (size %d out %d in %d)\n", __func__, size, out, in);

	if (kfifo_is_empty(&ads->in.fifo)) {
		D("%s: input fifo is empty\n", __func__);
		return 0;
	}

	if (size == 0) {
		D("%s: user buffer is full\n", __func__);
		return 0;
	}
	...
	if (out < in) {
		...
		D("%s: (out < in) downsampled (%d req, %d actual)"\
			" -> %d (size %d)\n", __func__,
...
again:
	bytes_ds = __downsample_to_user(ads, buf, out,
				...
				size,
				&bytes_consumed_from_fifo);
	D("%s: (out > in) downsampled (req %d act %d size %d) -> %d\n",
	...
	if (!bytes_ds) {
		...
		D("%s: not enough data till end of fifo\n", __func__);
...
again:
	if (ads->recording_cancelled ||
		...
		D("%s: recording has been cancelled (read %d bytes)\n",
				__func__, total);
		goto done_ok;
	}
	...
	D("%s: copied %d bytes to user, total %d/%d\n",
	...
	D("%s: done reading %d bytes, %d available\n", __func__,

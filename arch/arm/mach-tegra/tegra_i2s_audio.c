...
struct audio_out_stream {
	...
	void *pcm_out_buffer;
	dma_addr_t pcm_out_phys;
	struct kfifo pcm_out;
	struct completion pcm_out_completion;
...
static int i2s_set_channel_bit_count(unsigned long base, int sampling, int bitclk)
{
	u32 val;
	...
	pr_info("%s: I2S_I2S_TIMING_0 = %08x\n", __func__, val);
...
static int i2s_fifo_set_attention_level(unsigned long base, int fifo, unsigned level)
{
	u32 val;
...
static void i2s_set_left_right_control_polarity(unsigned long base, int high_low)
{
	u32 val = i2s_readl(base, I2S_I2S_CTRL_0);
...
{
	return i2s_writel(base, i2s_readl(base, I2S_I2S_STATUS_0), I2S_I2S_STATUS_0);
}
...
static inline phys_addr_t i2s_get_fifo_phy_base(unsigned long phy_base, int fifo)
{
...
static int setup_dma(struct audio_driver_state *ads)
{
	...
	ads->out.pcm_out_phys = dma_map_single(NULL, ads->out.pcm_out_buffer,
				PCM_OUT_BUFFER_SIZE, DMA_TO_DEVICE);
	BUG_ON(!ads->out.pcm_out_phys);
...
static void tear_down_dma(struct audio_driver_state *ads)
{
	...
	ads->out.tx_dma = NULL;
	dma_unmap_single(&ads->pdev->dev, ads->out.pcm_out_phys,
				PCM_OUT_BUFFER_SIZE,
				DMA_TO_DEVICE);
	ads->out.pcm_out_phys = 0;
}

static void dma_tx_complete_callback(struct tegra_dma_req *req)
{
	struct audio_out_stream *aos =
		(struct audio_out_stream *)req->dev;
	int count = req->bytes_transferred;

	pr_debug("%s bytes transferred %d\n", __func__, count);

	kfifo_skip(&aos->pcm_out, count);

	if (kfifo_avail(&aos->pcm_out) > PCM_OUT_THRESHOLD &&
			!completion_done(&aos->pcm_out_completion)) {
		pr_debug("%s: complete (%d avail)\n", __func__,
				kfifo_avail(&aos->pcm_out));
		complete(&aos->pcm_out_completion);
	}
...
static void resume_dma_playback(struct audio_out_stream *aos)
{
	...
	unsigned out = __kfifo_off(&aos->pcm_out, aos->pcm_out.out);
	unsigned in = __kfifo_off(&aos->pcm_out, aos->pcm_out.in);

	BUG_ON(!kfifo_len(&aos->pcm_out));

	req->source_addr = aos->pcm_out_phys + out;
	if (out < in)
		req->size = in - out;
	else
		req->size = kfifo_size(&aos->pcm_out) - out;

	dma_sync_single_for_device(NULL,
			aos->pcm_out_phys + out, req->size, DMA_TO_DEVICE);
	...
	pr_debug("%s resume playback (%d in fifo, writing %d, in %d out %d)\n",
			__func__, kfifo_len(&aos->pcm_out), req->size, in, out);

	i2s_fifo_set_attention_level(ads->i2s_base, I2S_FIFO_TX, I2S_FIFO_ATN_LVL_FOUR_SLOTS);
...
static irqreturn_t i2s_interrupt(int irq, void *data)
{
	struct audio_driver_state *ads =
		(struct audio_driver_state *)data;
	...
	if (status & I2S_I2S_FIFO_TX_QS) {
		...
		if (kfifo_avail(&out->pcm_out) > PCM_OUT_THRESHOLD &&
				!completion_done(&out->pcm_out_completion)) {
			pr_debug("%s: complete (%d avail)\n", __func__,
					kfifo_avail(&out->pcm_out));
			complete(&out->pcm_out_completion);
		}

		if (stop_playback_if_necessary(out))
			goto done;

		empty = i2s_get_fifo_full_empty_count(ads->i2s_base, I2S_FIFO_TX);

		len = kfifo_out(&out->pcm_out, fifo_buffer, empty * sizeof(u16));
		len /= sizeof(u16);

		written = 0;
		while (empty-- && written < len) {
			ads->pio_stats.tx_fifo_written += written * sizeof(u16);
			i2s_fifo_write(ads->i2s_base, I2S_FIFO_TX, fifo_buffer[written++]);
		}
	...
	if (status & I2S_I2S_FIFO_RX_QS) {
		int read = 0;
		while(i2s_get_fifo_full_empty_count(ads->i2s_base, I2S_FIFO_RX)) {
...
static ssize_t tegra_audio_write(struct file *file, const char __user *buf, size_t size, loff_t *off)
{
	ssize_t rc, total = 0;
	unsigned nw = 0;
	...
	pr_debug("%s: write %d bytes, %d available\n", __func__, size, kfifo_avail(&ads->out.pcm_out));

again:
	rc = kfifo_from_user(&ads->out.pcm_out, buf + total, size - total, &nw);
	if (rc < 0) {
	...
	total += nw;
	if (total < size) {
		pr_debug("%s: sleep (user %d total %d nw %d)\n", __func__, size, total, nw);
		wait_for_completion_interruptible(&ads->out.pcm_out_completion);
		pr_debug("%s: awake\n", __func__);
		goto again;
	}
			...
			unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd) {
	case TEGRA_AUDIO_OUT_PAUSE:
		stop_playback_if_necessary(&ads->out);
		break;
	case TEGRA_AUDIO_OUT_RESUME:
		start_playback_if_necessary(&ads->out);
		break;
	default:
...
ssize_t debugfs_read(struct file *file, char __user *buf, size_t size, loff_t *off)
{
	int rc = 0;
...
ssize_t debugfs_write(struct file *file, const char __user *buf, size_t size, loff_t *off)
{
	char cmd[5];
	int use_dma;
...
{
	struct resource *res;
	struct clk *i2s_clk, *audio_2x_clk, *dap_mclk;
	...
	state->i2s_base = (unsigned long)ioremap(res->start, res->end - res->start + 1);
	if (!state->i2s_base) {
	...
	audio_2x_clk = tegra_get_clock_by_name("audio_2x");
	if (!audio_2x_clk) {
		pr_err("%s: could not get audio_2x clock\n", __func__);
		return -EIO;
	}
	clk_enable(audio_2x_clk);
	...
	if (state->pdata->master)
		i2s_set_channel_bit_count(state->i2s_base, 44100, clk_get_rate(i2s_clk));
	...
	init_completion(&state->out.pcm_out_completion);
	spin_lock_init(&state->out.pcm_out_lock);
	state->out.pcm_out_phys = 0;
	state->out.tx_dma = NULL;

	state->out.pcm_out_buffer = kmalloc(PCM_OUT_BUFFER_SIZE, GFP_KERNEL | GFP_DMA);
	if (!state->out.pcm_out_buffer) {
		pr_err("%s: could not allocate pcm_out_buffer\n", __func__);
		return -ENOMEM;
	}
	...
	kfifo_init(&state->out.pcm_out, state->out.pcm_out_buffer, PCM_OUT_BUFFER_SIZE);

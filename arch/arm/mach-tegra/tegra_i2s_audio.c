...
static irqreturn_t i2s_interrupt(int irq, void *data)
{
	...
	if (status & I2S_I2S_FIFO_TX_QS) {
		...
		len = kfifo_out(&out->pcm_out, fifo_buffer,
				empty * sizeof(u16));
		len /= sizeof(u16);
		...
		const char __user *buf, size_t size, loff_t *off)
{
	...
	pr_debug("%s: write %d bytes, %d available\n", __func__,
			size, kfifo_avail(&ads->out.pcm_out));
...
static int tegra_audio_probe(struct platform_device *pdev)
{
	...
	state->out.pcm_out_buffer = kmalloc(PCM_OUT_BUFFER_SIZE,
			GFP_KERNEL | GFP_DMA);
	...
	kfifo_init(&state->out.pcm_out, state->out.pcm_out_buffer,
			PCM_OUT_BUFFER_SIZE);

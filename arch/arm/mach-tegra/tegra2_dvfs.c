...
static int tegra2_dvfs_rel_vdd_cpu_vdd_core(struct dvfs_rail *from,
	struct dvfs_rail *to)
{
	if (from->new_millivolts > from->millivolts &&
	    to->new_millivolts < from->new_millivolts + 50)
		return from->new_millivolts + 50;

	if (to->new_millivolts < from->millivolts + 50)
		return from->millivolts + 50;

	return to->new_millivolts;
}

/* vdd_aon must be within 170 mV of vdd_core */
static int tegra2_dvfs_rel_vdd_core_vdd_aon(struct dvfs_rail *from,
	struct dvfs_rail *to)
{
	return from->new_millivolts;
}

...
{
	unsigned long invalid_wake_irq_mask[IRQ_MASK_SIZE];
	int i;
	int irq;
	bool can_enter_lp0 = true;

	/* Check for any interrupts set as wake sources that are not
	   supported in lp0 mode, and refuse to enter lp0
	 */
	for (i = 0; i < IRQ_MASK_SIZE; i++)
		invalid_wake_irq_mask[i] = tegra_lp0_irq_mask[i] &
			~tegra_lp0_possible_irq_mask[i];

	for_each_set_bit(irq, invalid_wake_irq_mask, NR_IRQS) {
		struct irq_desc *desc = irq_to_desc(irq);
		const char *irq_name = "unknown";
		if (desc && desc->action && desc->action->name)
			irq_name = desc->action->name;

		pr_info("Can't enter LP0: irq %d (%s) is set as a wake irq\n",
			irq, irq_name);
		//can_enter_lp0 = false;
	}

	return can_enter_lp0;
}

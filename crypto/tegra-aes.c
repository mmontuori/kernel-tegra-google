...
struct tegra_aes_dev {
	...
	unsigned long flags;
	wait_queue_head_t op_complete_q;
	int op_complete;
	u32 *buf_in;
...
static unsigned int sg_get_nents(struct scatterlist *sg)
{
	struct scatterlist *sg_node = sg_next(sg);
	unsigned int sg_depth = 1;
...
static int sg_copy(struct scatterlist **sg, size_t *offset, void *buf,
	size_t buflen, int out)
{
	int bytes;
	int sg_nents = sg_get_nents(*sg);

	if (!out)
		bytes = sg_copy_to_buffer(*sg, sg_nents, buf, buflen);
	else
		bytes = sg_copy_from_buffer(*sg, sg_nents, buf, buflen);
	...
	int nblocks, int mode, bool upd_iv)
{
	...
	if (wait_event_timeout(dd->op_complete_q, dd->op_complete,
		msecs_to_jiffies(50)) == 0) {
			dev_err(dd->dev, "timed out.\n");
			return -ETIMEDOUT;
	}
	dd->op_complete = 0;
	return 0;
...
static void aes_init_hw(struct tegra_aes_dev *dd)
{
	u32 value, cmdq[2];
	...
	/* set the iv */
	memcpy(dd->ivkey_base+AES_MAX_KEY_SIZE+AES_HW_IV_SIZE, dd->iv,
		AES_HW_IV_SIZE);
	memcpy(dd->ivkey_base+AES_MAX_KEY_SIZE, dd->iv, AES_HW_IV_SIZE);
	...
	/* settable command to get key+iv into internal registers */
	value = 0;
...
static int tegra_aes_handle_req(struct tegra_aes_dev *dd)
{
	...
	int ret = 0, nblocks, nbytes, total;
	int needs_bounce = 0, in, out, count;
	dma_addr_t addr_in, addr_out;
	struct scatterlist *in_sg, *out_sg;
	bool use_upd_iv = false;
	...
	while (total) {
		...
		/* check for alignment */
		in = IS_ALIGNED((u32)in_sg->offset, sizeof(u32));
		out = IS_ALIGNED((u32)out_sg->offset, sizeof(u32));
		needs_bounce = !(in && out);

		if (needs_bounce) {
			dd->flags &= ~FLAGS_FAST;
			nbytes = sg_copy(&in_sg, &in_sg->offset,
				(void *)dd->buf_in, count, 0);
			count = nbytes;
			addr_in = (u32)dd->dma_buf_in;
			addr_out = (u32)dd->dma_buf_out;
		} else {
			...
			dd->flags |= FLAGS_FAST;
			count = min((int)in_sg->length, (int)dma_max);
			WARN_ON(in_sg->length != out_sg->length);
		}
		nblocks = count / AES_BLOCK_SIZE;
		...
		if (needs_bounce) {
			nbytes = sg_copy(&out_sg, &out_sg->offset,
				(void *)dd->buf_out, count, 1);
		} else {
	...
	/* clear the iv */
	memset(dd->ivkey_base + AES_MAX_KEY_SIZE+AES_HW_IV_SIZE,
		0, AES_HW_IV_SIZE);
...
out:
	...
	if (!dd->total) {
		...
		dd->req->base.complete(&dd->req->base, 0);
	}
...
static int tegra_aes_get_random(struct crypto_rng *tfm, u8 *rdata, unsigned int dlen)
{
	struct tegra_aes_dev *dd = aes_dev;
	int ret, nbytes, total = dlen, nblocks;
	bool use_upd_iv = false;
	u8 *dest = rdata;
	...
	while (total) {
		nbytes = min(total, AES_HW_DMA_BUFFER_SIZE_BYTES);
		nblocks = nbytes / AES_BLOCK_SIZE;
		if (nbytes % AES_BLOCK_SIZE)
			nblocks++;

		ret = aes_start_crypt(dd, (u32)dd->dma_buf_in, (u32)dd->dma_buf_out, nblocks,
			dd->flags, use_upd_iv);
		if (ret < 0) {
			dev_err(dd->dev, "aes_start_crypt fail(%d)\n", ret);
			return ret;
		}
		memcpy(dest, dd->buf_out, nbytes);
		dest += nbytes;
		total -= nbytes;
		use_upd_iv = true;
	}
...
static int tegra_aes_rng_reset(struct crypto_rng *tfm, u8 *seed, unsigned int slen)
{
	struct tegra_aes_dev *dd = aes_dev;
	...
	if (slen > AES_BLOCK_SIZE) {
		dev_err(dd->dev, "seed size too large");
		return -ENOMEM;
	}
...
static int tegra_aes_probe(struct platform_device *pdev)
{
	...
	init_waitqueue_head(&dd->op_complete_q);

	/* get the irq */
	err = request_threaded_irq(INT_VDE_BSE_V, NULL, aes_irq,
